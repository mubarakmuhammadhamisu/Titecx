...
import { Enrollment, Payment, Student } from '@/components/admin/mock-data';
...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([studentsData, enrollmentsData, paymentsData]) => {
      const student = (studentsData.students ?? []).find((s: Student) => s.id === studentId) ?? null;
      const enrollments = (enrollmentsData.enrollments ?? []).filter((e: Enrollment) => e.studentId === studentId);
      const payments = (paymentsData.payments ?? []).filter((p: Payment) => p.studentId === studentId);
      if (student) setData({ student, enrollments, payments });
      setLoading(false);
  ...
  const handleToggleBan = async () => {
    ...
    await fetch('/api/admin/students', {
      ...
      body: JSON.stringify({ id: studentId, isBanned: !data.student.isBanned }),
    });
    setData((prev) => prev ? { ...prev, student: { ...prev.student, isBanned: !prev.student.isBanned } } : null);
    setBanTarget(false);
  };
  ...
  const { student, enrollments: studentEnrollments, payments: studentPayments } = data;
    ...
    <div className="space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-gray-400">
                <div className="flex items-center gap-2"><Mail size={18} className="text-indigo-400" /><span>{student.email}</span></div>
                <div className="flex items-center gap-2"><Calendar size={18} className="text-indigo-400" /><span>Joined {format(new Date(student.joinDate), 'MMMM d, yyyy')}</span></div>
                <div className="flex items-center gap-2"><Award size={18} className="text-indigo-400" /><span>{completedCourses} courses completed</span></div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl bg-gradient-to-br from-purple-500/10 to-purple-600/5 border border-purple-500/20 p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex justify-between text-sm"><span className="text-gray-500">Converted</span><span className="text-emerald-400 font-semibold">{student.referrals_converted}</span></div>
              <div className="flex justify-between text-sm"><span className="text-gray-500">Total Commission</span><span className="text-amber-400 font-semibold">₦{student.total_commission_earned.toLocaleString()}</span></div>
            </div>
            <Link href="/admin/referrals" className="inline-flex items-center gap-1 text-xs text-indigo-400 hover:text-indigo-300 transition">View referral records →</Link>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {studentPayments.length > 0
          ...
          : <div className="rounded-lg border border-indigo-500/20 bg-gray-900/50 p-6 text-center"><p className="text-gray-400">No payments</p></div>
        }
      </div>
