...
import { Student } from '@/components/admin/mock-data';
...
export default function StudentsPage() {
  ...
  const isInactiveStudent = (student: Student): boolean => {
    ...
    return new Date(student.lastLogin) < thirtyDaysAgo;
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-gray-400">Manage and view student information and enrollment history.</p>
      </div>

      <FilterBar searchValue={searchTerm} onSearchChange={setSearchTerm} placeholder="Search by name or email..." />

      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <p className="text-sm text-gray-400">Showing {filteredStudents.length} of {students.length} students</p>
        ...
        {viewMode === 'table' ? (
          <AdminTable columns={studentColumns} data={filteredStudents} onRowClick={handleRowClick} />
        ) : (
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setDeleteTarget(null)} className="flex-1 rounded-lg border border-gray-600 px-4 py-2 font-medium text-gray-300 hover:bg-gray-800 transition">Cancel</button>
            <button onClick={handleDeleteStudent} className="flex-1 rounded-lg bg-red-600 hover:bg-red-700 px-4 py-2 font-medium text-white transition">Delete</button>
          </>
        }
      >
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setBanTarget(null)} className="flex-1 rounded-lg border border-gray-600 px-4 py-2 font-medium text-gray-300 hover:bg-gray-800 transition">Cancel</button>
            <button onClick={handleToggleBan} className={`flex-1 rounded-lg px-4 py-2 font-medium text-white transition ${banTarget?.isBanned ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-amber-600 hover:bg-amber-700'}`}>
      ...
      >
        <p className="text-gray-300 text-sm">
          {banTarget?.isBanned
            ? <>Are you sure you want to unban <span className="font-bold text-white">{banTarget?.name}</span>? They will regain access.</>
            : <>Are you sure you want to ban <span className="font-bold text-white">{banTarget?.name}</span>? They will lose access immediately.</>
          }
        </p>
      </Modal>
    </div>
