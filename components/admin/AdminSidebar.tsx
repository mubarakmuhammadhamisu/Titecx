...
import {
  Menu, X, LayoutDashboard, Users, BookOpen, LogIn,
  CreditCard, Ticket, Trophy, Settings, LogOut,
  Package, GitBranch, Coins, Video,
} from 'lucide-react';
...
const NAV = [
  ...
  { href: '/admin/video-providers', label: 'Video Providers',icon: Video },
    ...
    <>
      {isMobile && (
        <button
          ...
          className="fixed top-5 right-5 z-50 p-2 rounded-xl bg-gray-900 border border-indigo-500/30"
        >
      ...
      >
        <Link href="/admin" className="flex items-center gap-3 mb-8 group" onClick={() => isMobile && setIsOpen(false)}>
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center font-bold text-white shadow-lg shadow-indigo-500/40 overflow-hidden">
            <Image src="/im1.png" alt="Logo" width={40} height={40} className="rounded-full w-full h-full object-cover" />
          </div>
          <div className="flex flex-col">
            <span className="text-white font-bold text-sm">Titecx</span>
              ...
              <Link key={href} href={href} onClick={() => isMobile && setIsOpen(false)}>
                <motion.div
                  className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition relative
                    ${active ? 'bg-indigo-500/15 text-white' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-800/50'}`}
        ...
        <div className="pt-4 mt-4 border-t border-indigo-500/10 space-y-3">
          {user && (
            <div className="flex items-center gap-3 px-2">
              <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-xs font-bold text-white">
                {user.avatarUrl
